...
// and false otherwise.
//DONE 4. Write a method `isOddAndIsMultipleOfThree2` - if you used `isOdd` and `isMultipleofThree` in your last solution, don't use it
// this time. If you did not, write this method building on your last methods.
//DONE 5. Write a method `fizzMultipleofThree` that takes in an `int` n and prints "Fizz" if n is a multiple of three, and prints n otherwise.
//DONE 6. Write a method that takes in a `Person` and returns `true` if the person is from London, and `false` otherwise.
//DONE 7. Write a method that takes in a `Person` and prints the name if the person has a name longer than 5 characters, and otherwise prints
// "Name is too short."
